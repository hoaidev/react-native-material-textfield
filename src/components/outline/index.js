...
export default class Line extends PureComponent {
    ...
    borderProps() {
        ...
        if (disabled) {
            return { borderColor: baseColor, borderWidth: disabledLineWidth };
        }
        if (restricted) {
            return { borderColor: errorColor, borderWidth: activeLineWidth };
        }
        return {
    ...
    render() {
        ...
        let topLineContainerStyle = {
            transform: [{ scaleX: I18nManager.isRTL ? -1 : 1 }, { translateX: Animated.multiply(labelAnimation, lineOffset) }]
        };
